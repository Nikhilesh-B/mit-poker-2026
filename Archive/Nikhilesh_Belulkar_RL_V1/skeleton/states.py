...
class RoundState(namedtuple('_RoundState', ['button', 'street', 'pips', 'stacks', 'hands', 'board', 'previous_state'])):
    ...
    def showdown(self):
        ...
        '''
        # Use stack differences to infer pot distribution when possible.
        # If stacks are equal, treat as split pot.
        if self.stacks[0] == self.stacks[1]:
            return TerminalState([0, 0], self)
        delta = self.stacks[0] - self.stacks[1]
        # Winner is the one with more chips left.
        if delta > 0:
