...
class RoundState(namedtuple('_RoundState', ['button', 'street', 'pips', 'stacks', 'hands', 'board', 'previous_state'])):
    ...
    def proceed_street(self):
        ...
        return RoundState(button, new_street, [0, 0], self.stacks, new_hands, new_board, self)
    ...
    def proceed(self, action):
        ...
        if isinstance(action, DiscardAction):
            # IMPORTANT: Copy hands and board to avoid mutation issues in CFR traversal
            new_hands = [list(hand) for hand in self.hands]
            new_board = list(self.board)
            if len(new_hands[active]) != 0:
                new_board.append(new_hands[active].pop(action.card))
            state = RoundState((1 - active) % 2, self.street, self.pips, self.stacks, new_hands, new_board, self)
            return state
        ...
        if isinstance(action, CallAction):
            ...
            if self.button == 0:  # sb calls bb
                return RoundState(1, 0, [BIG_BLIND] * 2, [STARTING_STACK - BIG_BLIND] * 2, new_hands, new_board,self)
            # both players acted
            ...
            state = RoundState(self.button + 1, self.street, new_pips, new_stacks, new_hands, new_board, self)
        ...
        if isinstance(action, CheckAction):
            ...
            # let opponent act
            return RoundState(self.button + 1, self.street, self.pips, self.stacks, new_hands, new_board, self)
        ...
        new_stacks[active] -= contribution
        new_pips[active] += contribution
        return RoundState(self.button + 1, self.street, new_pips, new_stacks, new_hands, new_board, self)