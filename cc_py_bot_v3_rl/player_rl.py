...
class PlayerCeylan_v1(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        score = poker_utils_rl.poker_hand_scorer(my_cards, board_cards)
...
class PlayerHenry_v1(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        #===DISCARD PHASE===
        if DiscardAction in legal_actions:
            hand_ranks = [self.RANK_MAP[c.__str__()[0]] for c in my_cards]
            hand_suits = [c.__str__()[1] for c in my_cards]
            board_ranks = [self.RANK_MAP[c.__str__()[0]] for c in board_cards]
        ...
        if street == 0:
            ...
            # Pair
                ...
                pair_rank = max([r for r in hand_ranks if hand_ranks.count(
                    r) == 2])
                kicker = max([r for r in hand_ranks if r != pair_rank])
                strength = 0.55 + (pair_rank / 14) * 0.25 + (
                    kicker / 14) * 0.05
            # High card
            else:
                ...
                # Suited bonus
                max_suit_count = max(hand_suits.count("s"), 
                                     hand_suits.count("h"),
                                     hand_suits.count("d"), 
                                     hand_suits.count("c"))
                if max_suit_count == 3:
                    ...
                    strength += 0.06
                # Connected bonus
                gaps = [ranks_sorted[i] - ranks_sorted[i+1] for i in range(
                    len(ranks_sorted) - 1)]
            ...
            # Decision logic
            if continue_cost > 0:
                pot_odds = continue_cost / (pot_total + continue_cost) if (
                    pot_total + continue_cost) > 0 else 0

                if strength < 0.20:
                    if FoldAction in legal_actions:
                        return FoldAction()
                    return CheckAction(
                        ) if CheckAction in legal_actions else CallAction()
                elif strength < 0.35:
                    ...
                    if RaiseAction in legal_actions:
                        ...
                        target = int(min_raise + (max_raise - min_raise
                                                  ) * size_factor * 0.3)
        ...
        all_ranks = [self.RANK_MAP[c.__str__()[0]] for c in all_cards]
        all_suits = [c.__str__()[1] for c in all_cards]
        ...
        if has_flush or has_straight:
            ...
            bucket = "strong"
        elif len(sorted_counts) >= 2 and sorted_counts[
                0] == 2 and sorted_counts[1] == 2:
            bucket = "strong"
        ...
        if continue_cost == 0:
            ...
            pot_odds = continue_cost / (
                pot_total + continue_cost) if (
                    pot_total + continue_cost) > 0 else 0
            if bucket == "strong":
                if RaiseAction in legal_actions:
                    min_raise, max_raise = round_state.raise_bounds()
...
class PokerMath:
    ...
    def count_outs(my_cards, board_cards, RANK_MAP):
        """Calculate Outs"""
        all_cards = list(my_cards) + list(board_cards)
        all_ranks = [RANK_MAP[c.__str__()[0]] for c in all_cards]
        all_suits = [c.__str__()[1] for c in all_cards]
        my_ranks = [RANK_MAP[c.__str__()[0]] for c in my_cards]
        outs = 0
        draw_type = None
        ...
        board_rank_counts = {}
        for r in [RANK_MAP[c.__str__()[0]] for c in board_cards]:
...
class OpponentModel:
    ...
    def end_hand(self):
        ...
        pfr = sum(self.pfr_history) / len(self.pfr_history
                                          ) if self.pfr_history else 0
        fold_to_raise = (sum(self.fold_to_raise_history) / len(
            self.fold_to_raise_history
    ...
    def get_stats(self):
        ...
        return {
            'vpip': sum(self.vpip_history) / len(self.vpip_history),
            'pfr': sum(self.pfr_history) / len(
                self.pfr_history) if self.pfr_history else 0,
            'fold_to_raise': (sum(self.fold_to_raise_history) / len(
                self.fold_to_raise_history) 
                             if self.fold_to_raise_history else 0.5),
...
class PlayerHenry_v2(Bot):
    ...
    def handle_new_round(self, game_state, round_state, active):
        ...
        if self.hands_played % 50 == 0:
            ...
            # if stats:
            #     print(f": {stats['type']} (: {stats['confidence']:.2f})")
    ...
    def get_action(self, game_state, round_state, active):
        ...
        # PREFLOP
        if street == 0:
            return self._handle_preflop(round_state, active, 
                                        my_cards, continue_cost, pot_total)
        
        # POSTFLOP
        return self._handle_postflop_ev(round_state, active, 
                                        my_cards, board_cards, 
                                        continue_cost, pot_total, street)
    
    def _handle_preflop(self, round_state, active, my_cards, 
                        continue_cost, pot_total):

        hand_ranks = [self.RANK_MAP[c.__str__()[0]] for c in my_cards]
        hand_suits = [c.__str__()[1] for c in my_cards]
        ranks_sorted = sorted(hand_ranks, reverse=True)
        
        strength = self._calculate_preflop_strength(
            hand_ranks, hand_suits, ranks_sorted)
        ...
        if continue_cost > 0:
            if is_top20:
                ...
                if RaiseAction in legal_actions and random.random() < 0.75:
                    ...
                    target = int(min_raise + (
                        max_raise - min_raise) * size_factor * 0.5)
                    target = max(min_raise, min(target, max_raise))
                    return RaiseAction(target)
                if CallAction in legal_actions:
                    return CallAction()
                ...
                if not in_position:
                    ...
                    if strength >= 0.40:
                        pot_odds = continue_cost / (
                            pot_total + continue_cost) if pot_total > 0 else 0
    ...
    def _handle_postflop_ev(self, round_state, active, my_cards, board_cards, 
        ...
        bucket, made_hand_strength = self._evaluate_postflop(
            all_ranks, all_suits)
        ...
        outs, draw_type = self.poker_math.count_outs(
            my_cards, board_cards, self.RANK_MAP)
        ...
        if continue_cost == 0:
            ...
            if bucket in ["nuts", "very-strong", "strong"]:
                if RaiseAction in legal_actions:
                    ...
                    if random.random() < bet_prob:
                        min_raise, max_raise = round_state.raise_bounds()
                        multiplier = 1.2 if bucket == "nuts" \
                            else 1.0 if bucket == "very-strong" else 0.8
            ...
            # semi-strong
                ...
                if in_position and RaiseAction in legal_actions:
                    bluff_prob = 0.30
                    if exploit_adjustments and exploit_adjustments[
                            'bluff_more']:
                        bluff_prob = 0.50
                    if random.random() < bluff_prob:
                        min_raise, max_raise = round_state.raise_bounds()
            ...
            # weak card
                ...
                if in_position and random.random(
                        ) < 0.05 and RaiseAction in legal_actions:
                    min_raise, max_raise = round_state.raise_bounds()
                    return RaiseAction(min_raise)
                return CheckAction(
                    ) if CheckAction in legal_actions else FoldAction()
            ...
            pot_odds = self.poker_math.calculate_pot_odds(
                continue_cost, pot_total)
            ev = self.poker_math.calculate_ev(
                total_equity, continue_cost, pot_total)
            should_call_math = self.poker_math.should_call_mathematically(
                total_equity, pot_odds)

            if bucket in ["nuts", "very-strong", "strong"]:
                if RaiseAction in legal_actions:
                    raise_prob = 0.85 if bucket in [
                        "nuts", "very-strong"] else 0.60
                ...
                if should_call_math:
                    # EV > 0，call
                    if CallAction in legal_actions:
#                         print(f"""  [Draw Call] Outs:{outs} 
# Equity:{total_equity:.2%} PotOdds:{pot_odds:.2%} EV:+{ev:.1f}""")
                        return CallAction()
                else:
                    # EV < 0，fold（
    ...
    def _calculate_preflop_strength(self, hand_ranks, hand_suits, 
                                    ranks_sorted):
        strength = 0.0
        ...
        if len(set(hand_ranks)) == 1:
            ...
            max_suit_count = max(hand_suits.count(s) for s in [
                's','h','d','c'])
            if max_suit_count == 3:
                strength += 0.12
            elif max_suit_count == 2:
    ...
    def _evaluate_preflop_strength(self, my_cards):
        ...
        hand_suits = [c.__str__()[1] for c in my_cards]
        ranks_sorted = sorted(hand_ranks, reverse=True)
        return self._calculate_preflop_strength(
            hand_ranks, hand_suits, ranks_sorted)
...
class MyRLBot_v0():
    def __init__(self):
        ...
        path2 = r"mit-poker-2026\cc_py_bot_v2_rl\ppo_pokerbot_v0\\"
...
if __name__ == '__main__':
    run_bot(PlayerCeylan_v1(), parse_args())
