...
class PlayerSkeleton(Bot):
    ...
    def handle_new_round(self, game_state, round_state, active):
        ...
        # print(f'\n---- {self.round_num} ----')
        # print(f'Skeleton Starting Bankroll: {my_bankroll}')
...
class PlayerCeylan_v1(Bot):
    ...
    def handle_new_round(self, game_state, round_state, active):
        ...
        # print(f'\n---- {self.round_num} ----')
        # print(f'CeylanBot_v1 Starting Bankroll: {self.my_bankroll}')
...
class PlayerHenry_v1(Bot):
    ...
    def handle_new_round(self, game_state, round_state, active):
        ...
        # print(f'\n---- {self.round_num} ----')
        # print(f'HenryBot_v1 Starting Bankroll: {self.my_bankroll}')
    ...
    def _get_rank(self, card):
        """Helper to safely get rank regardless of card format"""
        char = str(card)[0]
        return self.RANK_MAP.get(char, 2)
    ...
    def get_action(self, game_state, round_state, active):
        ...
        # SAFE CARD PARSING
        hand_ranks = [self._get_rank(c) for c in my_cards]
        hand_suits = [str(c)[1] for c in my_cards]
        board_ranks = [self._get_rank(c) for c in board_cards]
        board_suits = [str(c)[1] for c in board_cards]

        # === FIXED DISCARD PHASE ===
        if DiscardAction in legal_actions:
            keep_values = []
            # Use dynamic range based on current hand size
            for i in range(len(my_cards)):
                val = 0.0
                r, s = hand_ranks[i], hand_suits[i]

                # Pair/Trip logic
                pair_count = hand_ranks.count(r)
                val += 15.0 if pair_count == 3 else (12.0 if pair_count == 2 else 0)
                
                # High card weighting
                val += 5.0 if r == 14 else (3.0 if r >= 12 else (1.5 if r >= 10 else 0))
                
                # Flush/Straight potential (simplified for robustness)
                if (hand_suits.count(s) + board_suits.count(s)) >= 3: val += 4.0
                
                keep_values.append(val)

            if not keep_values: # Fallback if hand is somehow empty
                return CheckAction() if CheckAction in legal_actions else list(legal_actions)[0]()
...
class MyRLBot():
    def __init__(self):
        self.model = PPO.load(r"C:\Users\DELL\Desktop\MIT MFin\3_IAP_2026\6.9630\mit-poker-2026\cc_py_bot_v2_rl\ppo_pokerbot_v1\best_model\best_model.zip")
