...
# !!! Can use pkrbot to evaluate hands.
...
def state_to_obs(game_state, round_state, active):
    discrete_obs = []
    cont_obs = []
    
    my_cards = card_mapper(round_state.hands[active])
    board_cards = card_mapper(round_state.board)
    
    hand_enc = evaluate_poker_hands_list(my_cards, board_cards)
    
    # Pad the main observation lists.
    while len(my_cards) < 3:
        my_cards.append([0, 0])
    while len(board_cards) < 6:
        board_cards.append([0, 0])
    ...
    # Cards
    hole_flat = list(chain.from_iterable(my_cards))
    board_flat = list(chain.from_iterable(board_cards))
    ...
    # Total Bankroll
    tot_bankroll = game_state.bankroll / STARTING_STACK
...
def decode_rl_action(rl_action, round_state):
    fccrd = rl_action[0]
