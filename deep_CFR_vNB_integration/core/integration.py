...
class NetworkMCCFRIntegration:
    ...
    def select_network_action(self, state, player: int):
        ...
        selected_idx = random.choices(
            range(len(legal_actions)), weights=probs)[0]
...
def test_single_state_integration():
    ...
    integration = NetworkMCCFRIntegration(
        network, mccfr, is_strategy_network=False)
    ...
    assert abs(
        total_prob - 1.0) < 0.001, f"Probabilities should sum to 1, got {total_prob}"
    ...
    print(
        f"    Network strategy: {len(comparison['network_strategy'])} actions")
    print(
        f"    Tabular strategy: {len(comparison['tabular_strategy'])} actions")
