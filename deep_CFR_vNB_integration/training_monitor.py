...
class TrainingMonitor:
    ...
    def log_iteration(self, iteration: int, metrics: Dict, verbose: bool = True):
        ...
        self.history['samples_strategy'].append(metrics.get('strategy_samples', 0))
    ...
    def log_training_complete(self):
        ...
        print(f"Best loss: {self.best_loss:.2f} at iteration {self.best_iteration}")
    ...
    def load_checkpoint(self, deep_cfr, checkpoint_path: str) -> int:
        ...
        deep_cfr.strategy_network.load_state_dict(checkpoint['strategy_network_state'])
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            'total_samples': (
                ...
                (self.history['samples_strategy'][-1] if self.history['samples_strategy'] else 0)
            )
        }
...
def plot_training_curves(history: Dict, save_path: str = None):
    ...
    # Strategy loss
    ax2 = axes[0, 1]
    ax2.plot(iterations, history['loss_strategy'], label='Strategy Π', color='green', alpha=0.7)
    ax2.set_xlabel('Iteration')
    ax2.set_ylabel('Loss')
    ax2.set_title('Strategy Network Loss')
    ax2.legend()
    ax2.set_yscale('log')
    ...
    # Sample counts
    ax3 = axes[1, 0]
    ax3.plot(iterations, [s/1000 for s in history['samples_p0']], label='P0 Samples', alpha=0.7)
    ax3.plot(iterations, [s/1000 for s in history['samples_p1']], label='P1 Samples', alpha=0.7)
    ax3.plot(iterations, [s/1000 for s in history['samples_strategy']], label='Strategy Samples', alpha=0.7)
